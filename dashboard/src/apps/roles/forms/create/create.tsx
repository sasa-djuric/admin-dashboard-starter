...
// Services
import { ID, Permissions } from '@startup/services';
...
const CreateRoleForm: React.FunctionComponent<CreateProps> = ({ id, isEditMode, onSubmit }) => {
	...
	function onGlobalPermissionChange(permission: string) {
		...
		const isAlreadyChecked = values.permissions.length
			? globalPermissions.every(permission => values.permissions.includes(permission as Permissions))
			: false;

		if (isAlreadyChecked) {
			const currentPermissions = values.permissions.filter(permission => !globalPermissions.includes(permission));

			setGlobalPermissions(permissions =>
				permissions.filter(globalPermission => globalPermission !== permission)
			);
			return setFieldValue('permissions', currentPermissions);
		}

		setGlobalPermissions(permissions => [...permissions, permission]);
		setFieldValue('permissions', uniq([...values.permissions, ...globalPermissions]));
	}

	function onPermissionChange(entity: string, permission: string) {
		const globalPermissions = entities.map(entity => `${entity.value}:${permission}`);
		const permissionValue = `${entity}:${permission}` as Permissions;
		const isAlreadyChecked = values.permissions.indexOf(permissionValue) >= 0;
		let currentPermissions: Array<string>;

		if (isAlreadyChecked) {
			currentPermissions = values.permissions.filter(permission => permission !== permissionValue);
		} else {
			currentPermissions = [...values.permissions, permissionValue];
		}

		const isGlobalChecked = currentPermissions.length
			? globalPermissions.every(permission => currentPermissions.includes(permission))
			: false;
	...
	function updateGlobalPermissions(currentPermissions: Array<string>) {
		let globalPermissions: { [permission: string]: Array<Permissions> } = {};
		...
		entities.forEach(entity => {
			permissions.forEach(permission => {
				...
				globalPermissions[permission.value].push(`${entity.value}:${permission.value}` as Permissions);
			});
		});

		Object.entries(globalPermissions).forEach(([permission, mappedPermissions]: [string, Array<Permissions>]) => {
			if (mappedPermissions.every(globalPermission => currentPermissions.includes(globalPermission))) {
				setGlobalPermissions(permissions => [...permissions, permission]);
			}
		});
	}
					...
					<Row style={{ padding: '6px 16px' }}>
						{permissions.map(permission => {
							return (
								<Col span={24 / 4}>
									<Checkbox
										checked={globalPermissions.includes(permission.value)}
										onChange={e => onGlobalPermissionChange(permission.value)}
									>
										{permission.label}
									</Checkbox>
								</Col>
							...
							<Row style={{ padding: '6px 16px' }}>
								{permissions.map(permission => {
									...
									const isChecked = !!values.permissions.find(
										checkedPermission => checkedPermission === currentPermission
									);
									return (
										<Col span={24 / 4}>
											<Checkbox
												checked={isChecked}
												onChange={e => onPermissionChange(entity.value, permission.value)}
											>
												{permission.label}
											</Checkbox>
										</Col>
